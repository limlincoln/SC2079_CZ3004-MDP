...
class Connect_PC_Client(threading.Thread):
    ...
    def __init__(self, name, img_results, shoot_signal, PC_close_signal):
    ...
    def run(self):
        ...
        while True:
            if not self.shoot_signal.empty():
                ...
                img_result_thread = Receive_Img_Results("Img Result Thread", client_socket, img_map_id, self.img_results)
                ...
                #print(res)
            elif not self.PC_close_signal.empty():
                self.PC_close_signal.get()
                break
            else:
                time.sleep(0.1)
...
class Receive_Img_Results(threading.Thread):
    ...
    def __init__(self, name, client_socket, img_map_id, results):
